...
resource "aws_route53_record" "logstash" {
  ...
  type = "A"
  ttl = "300"
  records = ["${element(aws_instance.logstash.*.public_ip, count.index)}"]
}
...
resource "null_resource" "logstash" {
  ...
  provisioner "local-exec" {
   ...
   provisioner "local-exec" {
    command = "cd ${path.module}/../../ansible/ && ansible-playbook -e ANSIBLE_HOST_KEY_CHECKING=False -i inventories/${var.cluster_vertical}_${var.cluster_app}.ini logstash.yml --extra-vars='hosts=${var.cluster_vertical} vertical_name=${var.cluster_vertical}'"
  }
}
...
terraform {
  backend "s3" {
    bucket = "tfplivo"
    key    = "staging/service/tflogstash/terraform.tfstate"
    region = "ap-south-1"
  }
}